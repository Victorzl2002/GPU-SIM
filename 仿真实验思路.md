...
- 若需要“不同负载下的各方案对比”，在原命令后追加 `--load-sweep 120 160 200`（可填多个任务数）。脚本会对每个场景分别跑这些 num_tasks，JSON/图表名称会变成 `A3-sandbox_tasks160_report.json` 这类形式，并自动绘制“X 轴=任务数、Y 轴=SLO/利用率/Drop rate/限流事件”的折线图，方便直接观察随负载变化的趋势。
- 产物（与您打开的图一一对应）：
  - `ir_cdf.png`（全场景 IR CDF 对比）
  - `<SCENARIO>_ir_hist.png`（单场景 IR 直方图，带负载后缀）
  - `slo_rate.png`、`compute_util.png`（SLO 与平均算力利用率折线；负载 sweep 时改为“不同负载”曲线）
  - `drop_rate.png`、`limiter_events.png`（丢弃率与限流次数；负载 sweep 时改为按负载绘制）
...
- `scripts/exp_coeff_sweep.py`：遍历 α/β/γ 折算系数组合，同时对基线 (`--baseline-scenario`) 与沙盒 (`--sandbox-scenario`) 场景执行仿真，输出 SLO / IR / Drop/限流的“沙盒相对基线提升”。脚本会将这些提升绘制成 `ΔSLO vs ΔIR>1.25` 气泡图，便于检查在不同系数下沙盒结论是否稳定。示例：

  ```bash
  python3 scripts/exp_coeff_sweep.py \
    --baseline-scenario A1-baseline \
    --sandbox-scenario A3-sandbox \
    --loads 150 190 230 \
    --seeds 7 13 \
    --output-root reports/coeff_sweep_v1
  ```

  可通过 `--coeff-set baseline:0.85,0.90,0.80` 追加自定义组合，或修改 `--vendor/--model` 指定其它 GPU 系数。

- `scripts/tune_sandbox.py`：联合搜索三门阈值与 SLO 守护力度，自动对每个组合打分并输出 CSV/气泡图。示例：

  ```bash
  python3 scripts/tune_sandbox.py \
    --scenario-template A3-sandbox \
    --loads 150 190 230 \
    --seeds 7 \
    --limit-thresholds 0.95 1.00 1.05 1.10 \
    --bandwidth-refill 0.7 0.85 \
    --compute-ceiling 1.3 1.4 \
    --max-boost 1.2 1.4 \
    --decay 0.02 \
    --adjust-interval 12 16 \
    --output-root reports/sandbox_tuning_v1
  ```

  默认评分函数为 `score = SLO - w_ir·IR>1 - w_limit·限流比例 - w_drop·Drop`，可用 `--weight-*` 调整惩罚系数。脚本会把所有组合的指标写入 `sandbox_summary.csv` 和气泡图，便于挑出最优参数。
